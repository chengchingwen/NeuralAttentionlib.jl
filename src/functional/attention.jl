@inline function generic_qkv_attention(mixingf, scoref, q::AbstractArray, k::AbstractArray, v::AbstractArray, args...)
    return unwrap_collapse(mixing(mixingf, v, scoref, q, k, args...))
end

@inline function generic_multihead_qkv_attention(mixingf, scoref, head::Integer, q::AbstractArray, k::AbstractArray, v::AbstractArray, args...)
...
"""
@inline naive_qkv_attention(q::AbstractArray, k::AbstractArray, v::AbstractArray, mask=nothing) = unwrap_collapse(weighted_sum_mixing(
    normalized_score(NNlib.softmax, masked_score, mask, scaled_dot_product_score, q, k),
    v))

@inline function multihead_qkv_attention(head::Integer, q::AbstractArray, k::AbstractArray, v::AbstractArray, mask=nothing)
    hq = CollapsedDimArray((move_head_dim_out ∘ split_head)(head, q), static(2), static(ndims(q)))
    hk = CollapsedDimArray((move_head_dim_out ∘ split_head)(head, k), static(2), static(ndims(k)))
    hv = CollapsedDimArray((move_head_dim_out ∘ split_head)(head, v), static(2), static(ndims(v)))
...
attention(head, q, k, v, mask=nothing) = multihead_qkv_attention(head, q, k, v, mask)
attention(q, k, v, mask=nothing) = naive_qkv_attention(q, k, v, mask)
